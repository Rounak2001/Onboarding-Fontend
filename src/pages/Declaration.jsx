...
const Declaration = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            syncAuthData(data);
            navigate(getNextRoute(data), { replace: true });
        } catch (err) {
            console.error('Error accepting declaration:', err);
            setError(err?.response?.data?.error || 'Failed to submit declaration. Please try again.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div style={{ marginTop: 32, display: 'flex', justifyContent: isPhoneScreen ? 'stretch' : 'flex-end', paddingTop: 24, borderTop: '1px solid #e5e7eb' }}>
                <button
                    ...
                    style={{
                        ...
                        fontSize: 14,
                        border: 'none',
                        cursor: allAgreed && !loading ? 'pointer' : 'not-allowed',
