...
const ConsultantDetail = () => {
    ...
    const fetchPerformanceData = async (force = false) => {
        if (performanceData && !force) return;
        setLoadingPerformance(true);
        try {
            const res = await fetch(apiUrl(`/admin-panel/consultants/${id}/performance/`), {
                headers: token ? { 'Authorization': `Bearer ${token}` } : {},
            });
            if (res.ok) {
                const d = await res.json();
    ...
    const titleContainerStyle = { display: 'flex', alignItems: 'center', gap: 12 };
    const titleStyle = { fontWeight: 700, fontSize: 15, color: 'var(--admin-text-primary)', margin: 0 };
    const iconStyle = { fontSize: 18 };

    const contentStyle = { padding: isMobile ? '16px 12px' : '20px' };
    const gridStyle = { display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(240px, 1fr))', gap: 24 };
    const fieldStyle = { display: 'flex', flexDirection: 'column', gap: 6 };
    const labelStyle = { fontSize: 12, fontWeight: 700, color: 'var(--admin-text-muted)', textTransform: 'uppercase', letterSpacing: 0.5 };
    const valueStyle = { fontSize: 15, fontWeight: 600, color: 'var(--admin-text-strong)' };
    ...
    return (
        <>
            ...
            <div style={{ maxWidth: 1500, margin: '0 auto', padding: isMobile ? '14px 10px 18px' : '28px 32px' }}>
                ...
                {/* ===== VIEW MODE TOGGLE ===== */}
                <div style={{
                    display: 'flex', gap: 10, marginBottom: 20, padding: '4px',
                    background: 'var(--admin-header-bg)', borderRadius: 10,
                    width: 'fit-content', border: '1px solid var(--admin-border-soft)'
                }}>
                    <button
                        onClick={() => setViewMode('onboarding')}
                        style={{
                            padding: '8px 16px', borderRadius: 6, fontSize: 13, fontWeight: 600,
                            background: viewMode === 'onboarding' ? '#3b82f6' : 'transparent',
                            color: viewMode === 'onboarding' ? '#fff' : 'var(--admin-text-secondary)',
                    ...
                    <button
                        ...
                        style={{
                            padding: '8px 16px', borderRadius: 6, fontSize: 13, fontWeight: 600,
                            background: viewMode === 'consultant' ? '#10b981' : 'transparent',
                            color: viewMode === 'consultant' ? '#fff' : 'var(--admin-text-secondary)',
                            border: 'none', cursor: 'pointer', transition: 'all 0.2s'
                        }}
                    >
                        Consultant Mode
                    ...
                    <div style={{ animation: 'fadeIn 0.3s ease' }}>
                        {loadingPerformance ? (
                            <div style={{ padding: 40, textAlign: 'center', color: 'var(--admin-text-muted)' }}>
                                <div style={{
                                    width: 36, height: 36, border: '3px solid var(--admin-border-strong)', borderTopColor: '#10b981',
                ...
                {viewMode === 'onboarding' && (
                    <div style={{ animation: 'fadeIn 0.3s ease', display: 'flex', flexDirection: 'column', gap: 24 }}>
                        {/* Profile Completion Checklist (Quick View) */}
                        {performanceData && performanceData.profile_completion && (
                            <div style={{
                                padding: 20, borderRadius: 16, background: 'var(--admin-surface)',
                                border: '1px solid var(--admin-border-soft)',
                            }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                                    <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--admin-text-strong)' }}>Onboarding Progress</h3>
                                    <span style={{ fontSize: 18, fontWeight: 800, color: '#10b981' }}>{performanceData.profile_completion.percentage}%</span>
                                </div>
                                <div style={{ height: 6, borderRadius: 3, background: 'var(--admin-border-soft)', overflow: 'hidden', marginBottom: 16 }}>
                                    <div style={{ height: '100%', width: `${performanceData.profile_completion.percentage}%`, background: '#10b981', transition: 'width 0.5s ease' }} />
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 8 }}>
                                    {(performanceData.profile_completion.steps || []).slice(0, 4).map(step => (
                                        <div key={step.key} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
                                            {step.completed ? <CheckCircle2 size={14} color="#10b981" /> : <Circle size={14} color="var(--admin-text-muted)" opacity={0.5} />}
                                            <span style={{ color: step.completed ? 'var(--admin-text-muted)' : 'var(--admin-text-primary)' }}>{step.label}</span>
                                        </div>
                                    ))}
                                    {(performanceData.profile_completion.steps || []).length > 4 && (
                                        <div style={{ fontSize: 12, color: '#10b981', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }} onClick={() => setViewMode('consultant')}>
                                            View all steps <ChevronRight size={14} />
                                        </div>
                                    )}
                                </div>
                            </div>
                                ...
                                <div style={{ padding: isMobile ? '12px 12px 16px' : '12px 20px 20px' }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 0 : '0 40px' }}>
                                        <div>
                                            ...
                                            {fieldRow('Age', p.age)}
                                            {fieldRow('Gender', p.gender_label || p.gender)}
