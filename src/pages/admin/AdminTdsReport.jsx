...
import { IndianRupee, Users, Megaphone, RefreshCw, Download, AlertTriangle } from 'lucide-react';
...
const AdminTdsReport = ({ isLight, viewportWidth, token, themeVars }) => {
    ...
    }, [authHeaders, financialYear, navigate]);
    ...
    const downloadReport = async (format = 'csv') => {
        ...
        try {
            ...
            a.href = url;
            a.download = `tds_report_${financialYear}.${format}`;
    ...
    const summaryCards = [
        ...
        { key: 'consultant', label: 'Consultants', value: stats.consultant_count || 0, color: '#f59e0b', icon: Users, sub: 'Payout payees (estimated TDS)' },
    ];

    return (
        <div style={{ ...themeVars }}>
            <div style={{
                padding: '12px 16px', marginBottom: 20, borderRadius: 14,
                background: isLight ? 'rgba(245,158,11,0.08)' : 'rgba(245,158,11,0.1)',
                border: '1px solid rgba(245,158,11,0.25)',
                display: 'flex', alignItems: 'flex-start', gap: 10,
            }}>
                <AlertTriangle size={16} color="#f59e0b" style={{ flexShrink: 0, marginTop: 2 }} />
                <div style={{ fontSize: 12.5, color: 'var(--admin-text-secondary)', lineHeight: 1.5 }}>
                    Ambassador TDS figures come from stored commission records. Consultant payout TDS is an
                    <strong style={{ color: '#f59e0b' }}> estimated 10% rate</strong> — the platform does not
                    currently store an actual TDS rate for consultant payouts. Confirm the real rate with Finance/Compliance before filing.
                </div>
            </div>
            ...
            <div style={{ background: 'var(--admin-surface)', borderRadius: 20, border: '1px solid var(--admin-border-soft)', overflow: 'hidden' }}>
                <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--admin-border-soft)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: 'var(--admin-text-primary)' }}>TDS by Payee — FY {financialYear}</h3>
                    {payeeFilter !== 'all' && (
                ...
                <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan={7} style={emptyStyle}><RefreshCw className="spin" size={22} style={{ marginBottom: 10, opacity: 0.5 }} /><div>Loading TDS report...</div></td></tr>
                            ) : error ? (
                                <tr><td colSpan={7} style={emptyStyle}>{error}</td></tr>
                                ...
                                filteredRows.map((r) => (
                                    <tr key={`${r.payee_type}-${r.payee_id}`} style={{ borderBottom: '1px solid var(--admin-border-soft)' }}>
                                        ...
                                        <td style={{ ...tdStyle, textAlign: 'right' }}>
                                            {r.tds_rate_pct}%{r.rate_is_estimated && <span title="Estimated — no stored TDS rate for this payee type" style={{ color: '#f59e0b', marginLeft: 4 }}>*</span>}
                                        </td>
                                ...
                                <tr><td colSpan={7} style={emptyStyle}>No TDS records found for FY {financialYear}.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
