...
const CallLogs = () => {
    ...
    const [activeTab, setActiveTab] = useState('service'); // 'service' or 'onboarding'
    ...
    const fetchLogs = useCallback(async (pg = 1, searchQuery = search, status = statusFilter, outcome = outcomeFilter, tab = activeTab, dateRange = dateRangeFilter) => {
        setLoading(true);
        setError('');
        try {
    ...
    }, [authHeaders, navigate, search, statusFilter, outcomeFilter, activeTab, dateRangeFilter]);
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', position: 'relative', background: 'var(--admin-page-bg)' }}>
                ...
                <div style={{ 
                    ...
                    flexShrink: 0,
                     ...
                     {[
                         ...
                         return (
                             <button key={s.range} onClick={() => { const next = isActive ? 'all' : s.range; setDateRangeFilter(next); fetchLogs(1, search, statusFilter, outcomeFilter, activeTab, next); }} style={{ flexShrink: 0, minWidth: 90, padding: '12px 20px', background: isActive ? `${s.color}18` : 'var(--admin-surface)', borderRadius: 20, border: `1px solid ${isActive ? s.color : 'var(--admin-border-soft)'}`, display: 'flex', flexDirection: 'column', gap: 4, boxShadow: '0 4px 12px rgba(0,0,0,0.02)', textAlign: 'center', cursor: 'pointer', transition: 'all 0.18s' }}>
                     ...
                     {[
                         ...
                         return (
                             <button key={s.status} onClick={() => { const next = isActive ? 'all' : s.status; setStatusFilter(next); fetchLogs(1, search, next, outcomeFilter, activeTab, dateRangeFilter); }} style={{ flexShrink: 0, minWidth: 90, padding: '12px 20px', background: isActive ? `${s.color}18` : 'var(--admin-surface)', borderRadius: 20, border: `1px solid ${isActive ? s.color : 'var(--admin-border-soft)'}`, display: 'flex', flexDirection: 'column', gap: 4, boxShadow: '0 4px 12px rgba(0,0,0,0.02)', textAlign: 'center', cursor: 'pointer', transition: 'all 0.18s' }}>
                ...
                <div style={{ padding: '0 32px', background: 'var(--admin-page-bg)' }}>
                    <div style={{ display: 'flex', gap: 24, borderBottom: '1px solid var(--admin-border-soft)' }}>
                        <button 
                            onClick={() => { setLogs([]); setStats(null); setActiveTab('service'); }}
                            style={{ 
                                padding: '16px 8px', border: 'none', background: 'none', 
                                borderBottom: activeTab === 'service' ? '2px solid #3b82f6' : '2px solid transparent',
                                color: activeTab === 'service' ? '#3b82f6' : 'var(--admin-text-muted)',
                        ...
                        <button 
                            onClick={() => { setLogs([]); setStats(null); setActiveTab('onboarding'); }}
                            style={{ 
                                padding: '16px 8px', border: 'none', background: 'none', 
                                borderBottom: activeTab === 'onboarding' ? '2px solid #3b82f6' : '2px solid transparent',
                                color: activeTab === 'onboarding' ? '#3b82f6' : 'var(--admin-text-muted)',
                                fontWeight: 700, fontSize: 14, cursor: 'pointer', transition: 'all 0.2s'
                            }}
                        >
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                                            <div style={{ display: 'flex', gap: 16, flex: 1, minWidth: 280 }}>
                                                <div style={{ 
                                                    ...
                                                    background: activeTab === 'service' ? 'linear-gradient(135deg, #3b82f615 0%, #3b82f605 100%)' : 'linear-gradient(135deg, #10b98115 0%, #10b98105 100%)', 
                                                    display: 'flex', alignItems: 'center', justifyContent: 'center', 
                                                    color: activeTab === 'service' ? '#3b82f6' : '#10b981', 
                                                    flexShrink: 0,
                                                    border: `1px solid ${activeTab === 'service' ? '#3b82f620' : '#10b98120'}`
                                                }}>
                                                    {activeTab === 'service' ? <PhoneOutgoing size={26} /> : <PhoneIncoming size={26} />}
                                                </div>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                                        {activeTab === 'service' ? (
                                                            <>
                                                                ...
                                                                <span 
                                                                    onClick={() => navigate(`/Clients/${log.client_id}`)}
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                                                        <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--admin-text-muted)', fontWeight: 500 }}>
                                                            <Calendar size={14} /> {formatDate(activeTab === 'service' ? log.created_at : log.called_at)}
                                                        </span>
                                                        {activeTab === 'service' && (
                                                            <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--admin-text-muted)', fontWeight: 500 }}>
                                                        ...
                                                        {activeTab === 'onboarding' && log.call_round && (
                                            ...
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', justifyContent: isMobile ? 'flex-start' : 'flex-end' }}>
                                                ...
                                                <a 
                                                    href={`tel:${activeTab === 'service' ? (log.client_phone || '') : (log.consultant_phone || '')}`} 
                                        ...
                                        {(log.notes || log.comments) && (() => {
                                            ...
                                            return (
                                                <div style={{ 
                                                    ...
                                                    borderLeft: `4px solid ${activeTab === 'service' ? '#3b82f6' : '#10b981'}`,
                                                    lineHeight: 1.6
                                                }}>
                                                    <div style={{ fontWeight: 800, fontSize: 11, color: 'var(--admin-text-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                                        {activeTab === 'service' ? 'Consultant Notes' : 'Admin Comments'}
                                                    </div>
